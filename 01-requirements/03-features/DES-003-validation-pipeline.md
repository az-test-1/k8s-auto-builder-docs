...
API 검증의 핵심 가치는 **컴파일/시작 단계에서 발견되지 않는 런타임 에러**를 탐지하는 것입니다.

#### 6.6.1 검증 단계별 에러 탐지 범위
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Phase 1: BUILD        Phase 3: STARTUP       Phase 4: API         │
│  (컴파일 타임)          (초기화 타임)           (런타임)              │
│                                                                     │
│  ✓ 문법 에러           ✓ Bean 생성 실패       ✓ 비즈니스 로직 에러   │
│  ✓ 타입 불일치         ✓ 의존성 주입 실패     ✓ NullPointerException │
│  ✓ import 누락         ✓ DB 연결 실패         ✓ ClassCastException  │
│  ✓ 메서드 시그니처     ✓ 설정 파싱 에러       ✓ SQL 런타임 에러     │
│                        ✓ Port 충돌            ✓ 직렬화/역직렬화 에러 │
│                                               ✓ 권한/인증 에러      │
│                                               ✓ 트랜잭션 에러       │
│                                               ✓ 레이지 로딩 에러    │
│                                                                     │
│  커버리지: ~60%        커버리지: ~75%         커버리지: ~90%+       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

#### 6.6.2 Java 8→17 / Spring Boot 2→3 마이그레이션 런타임 에러

| 에러 유형 | 원인 | 컴파일 | 시작 | API |
|----------|------|:------:|:----:|:---:|
| `NoClassDefFoundError` | javax → jakarta 누락 | ✗ | △ | ✓ |
| `ClassNotFoundException` | 제거된 클래스 사용 | ✗ | △ | ✓ |
| `NoSuchMethodError` | 메서드 시그니처 변경 | ✗ | △ | ✓ |
| `QuerySyntaxException` | Hibernate 6 쿼리 문법 | ✗ | ✗ | ✓ |
| `JsonMappingException` | Jackson 직렬화 변경 | ✗ | ✗ | ✓ |
| `AccessDeniedException` | Spring Security 변경 | ✗ | ✗ | ✓ |
| `LazyInitializationException` | JPA 세션 관리 변경 | ✗ | ✗ | ✓ |

> △: 특정 조건에서만 탐지, ✓: 확실히 탐지, ✗: 탐지 불가

#### 6.6.3 런타임 에러 예시

**1. javax → jakarta 네임스페이스 (Spring Boot 3)**

```java
// 컴파일 OK, 시작 OK, API 호출 시 실패
@PostMapping("/users")
public Response createUser(@Valid @RequestBody UserDto dto) {
    // @Valid가 javax.validation을 참조하면
    // 런타임에 NoClassDefFoundError: javax/validation/Valid
}
```

**2. Hibernate 6 쿼리 문법 변경**

```java
// 컴파일 OK, 시작 OK, 쿼리 실행 시 실패
@Query("SELECT u FROM User u WHERE u.deletedAt IS NULL")
List<User> findActiveUsers();
// Hibernate 6에서 특정 표현식 문법 변경 시 QuerySyntaxException
```

**3. Jackson 직렬화 변경**

```java
// 컴파일 OK, 시작 OK, JSON 응답 시 실패
@GetMapping("/users/{id}")
public UserResponse getUser(@PathVariable Long id) {
    return userService.findById(id);
    // LocalDateTime 필드가 새 Jackson에서 직렬화 실패
    // HttpMessageNotWritableException
}
```

**4. Spring Security 필터 체인**

```java
// 컴파일 OK, 시작 OK, API 호출 시 403
@GetMapping("/api/data")
@PreAuthorize("hasRole('USER')")
public Data getData() {
    // Spring Security 6에서 ROLE_ prefix 처리 변경
    // 기존: hasRole('USER') → ROLE_USER 자동 추가
    // 변경: 명시적 처리 필요한 케이스 존재
}
```
...
```python
# validation/runtime_error_classifier.py
class RuntimeErrorClassifier:
    """API 응답에서 런타임 에러 유형 분류"""

    ERROR_PATTERNS = {
        "CLASS_NOT_FOUND": [
            r"NoClassDefFoundError",
            r"ClassNotFoundException",
            r"javax\.",  # javax 패키지 참조
        ],
        "METHOD_NOT_FOUND": [
            r"NoSuchMethodError",
        ...
        "DATABASE_ERROR": [
            ...
            r"LazyInitializationException",
        ],
        "SERIALIZATION_ERROR": [
            r"JsonMappingException",
            r"JsonParseException",
            r"HttpMessageNotWritableException",
        ...
        "TYPE_ERROR": [
            ...
            r"MethodArgumentTypeMismatchException",
        ],
    }

    def classify(self, response) -> RuntimeErrorInfo:
        """응답에서 에러 유형 분류"""
        error_body = response.text if hasattr(response, 'text') else str(response)
        status_code = getattr(response, 'status_code', 500)

        error_info = RuntimeErrorInfo(
            status_code=status_code,
            raw_message=error_body[:2000],  # 로그 크기 제한
        )
        ...
        if not error_info.error_type:
            error_info.error_type = "UNKNOWN_RUNTIME_ERROR"

        # 스택트레이스 추출
        error_info.stack_trace = self._extract_stack_trace(error_body)

        # 관련 클래스/파일 추출
        error_info.related_classes = self._extract_class_names(error_body)

        return error_info

    def _extract_stack_trace(self, body: str) -> List[str]:
        """스택트레이스에서 프로젝트 코드만 추출"""
        lines = []
        for line in body.split('\n'):
            # 프로젝트 패키지 경로만 추출 (com.example, com.company 등)
            if re.match(r'\s+at (com\.|org\.company)', line):
                lines.append(line.strip())
        return lines[:10]  # 상위 10개만

    def _extract_class_names(self, body: str) -> List[str]:
        """에러에서 관련 클래스명 추출"""
        # com.example.UserController.getUser 형태 추출
        pattern = r'(com\.[a-zA-Z0-9_.]+)\.'
...
```python
# validation/runtime_error_fixer.py
class RuntimeErrorFixer:
    """런타임 에러 발견 시 LLM 자동 수정"""

    FIX_HINTS = {
        "CLASS_NOT_FOUND": """
            javax → jakarta 네임스페이스 마이그레이션이 누락되었습니다.
            - javax.validation → jakarta.validation
            - javax.persistence → jakarta.persistence
            - javax.servlet → jakarta.servlet
            import문과 의존성(pom.xml)을 모두 확인하세요.
        """,
        "METHOD_NOT_FOUND": """
            메서드 시그니처가 변경되었습니다.
            Spring Boot 3 / Java 17에서 변경된 API를 확인하세요.
            대체 메서드나 새로운 방식을 적용해야 합니다.
        """,
        "DATABASE_ERROR": """
            Hibernate 6 / Spring Data JPA 변경사항:
            - JPQL 문법 변경 확인
            - @Query 어노테이션 검토
            - 엔티티 매핑 검증
        """,
        "SERIALIZATION_ERROR": """
            Jackson 직렬화 설정 변경:
            - LocalDateTime: @JsonFormat 또는 JavaTimeModule 등록
            - Optional: 적절한 직렬화 처리
            - 새 필드: @JsonIgnore 또는 기본값 설정
        """,
        "SECURITY_ERROR": """
            Spring Security 6 변경사항:
...
### 에러 메시지
{error_info.raw_message[:1000]}

### 스택트레이스
{chr(10).join(error_info.stack_trace)}

### 힌트
{self.FIX_HINTS.get(error_info.error_type, '')}

### 관련 소스 코드
{await self._read_files(related_files)}

### 요청
위 런타임 에러를 수정하는 코드 패치를 생성해주세요.
Java 17 / Spring Boot 3 호환 코드로 작성하세요.
"""

        # 3. LLM 수정 요청
        fix_response = await self.llm_manager.generate(prompt)

        # 4. 패치 파싱 및 적용
        patches = self._parse_patches(fix_response)

        return FixResult(
            error_type=error_info.error_type,
            patches=patches,
            confidence=self._calculate_confidence(error_info, patches)
        )
    ...
    async def _find_related_files(
        ...
        for class_name in class_names:
            # com.example.UserController → UserController.java
            simple_name = class_name.split('.')[-1]
            pattern = f"**/{simple_name}.java"

            found = glob.glob(f"{workspace}/{pattern}", recursive=True)
            files.extend(found)

        return list(set(files))[:5]  # 최대 5개 파일
```

#### 6.6.6 API 검증 + 런타임 에러 수정 통합 흐름

```python
# validation/api_validator.py (확장)
class ApiValidator:
    def __init__(self):
        self.error_classifier = RuntimeErrorClassifier()
        self.error_fixer = RuntimeErrorFixer()

    async def validate_with_auto_fix(
        self,
        context: ValidationContext,
        max_fix_attempts: int = 3
    ) -> ValidationResult:
        """런타임 에러 자동 수정 포함 API 검증"""
        ...
        for attempt in range(max_fix_attempts):
            ...
            # 3. 런타임 에러 수정 시도
            self.logger.info(f"런타임 에러 {len(runtime_errors)}개 발견, 수정 시도 #{attempt + 1}")

            for error_result in runtime_errors:
                # 에러 분류
                error_info = self.error_classifier.classify(error_result.response)

                # 자동 수정
                fix_result = await self.error_fixer.fix(error_info, context)

                if fix_result.patches:
                    # 패치 적용
                    await self._apply_patches(fix_result.patches, context)

            # 4. 앱 재시작 (수정 사항 반영)
            await self._restart_app(context)

        # 최대 시도 후에도 실패
        result.success = False
        result.message = f"{max_fix_attempts}회 수정 시도 후에도 런타임 에러 존재"
        return result
```
  ...
  "runtime_errors": [
    {
      ...
      "matched_pattern": "NoClassDefFoundError",
      "message": "java.lang.NoClassDefFoundError: javax/validation/Valid",
      "related_classes": [
      ...
      "stack_trace": [
        ...
        "at com.example.service.UserService.validate(UserService.java:32)"
      ],
      "fix_applied": true,
      "fix_description": "javax.validation → jakarta.validation 변경"
    }
  ],
  "fix_summary": {
    "total_errors": 3,
...
#### 6.6.8 런타임 에러 탐지 환경 변수

| 변수명 | 설명 | 기본값 |
|--------|------|--------|
| `VALIDATION_RUNTIME_FIX_ENABLED` | 런타임 에러 자동 수정 활성화 | `true` |
| `VALIDATION_RUNTIME_FIX_MAX_ATTEMPTS` | 최대 수정 시도 횟수 | `3` |
| `VALIDATION_RUNTIME_ERROR_PATTERNS` | 추가 에러 패턴 (JSON) | `{}` |
